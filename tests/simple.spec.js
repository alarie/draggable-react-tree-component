...
describe('simple tree', () => {
  ...
  it('should have checkbox, default expand all treeNode, not show icon, show line', (done) => {
    ...
    setTimeout(() => {
      ...
      // have checkbox
      expect(dom.find('.react-tree-checkbox').length).to.be(5);
      // expand all treeNode
      dom.find('.react-tree-switcher').each((index, item) => {
        // console.log(item);
      ...
      // not show icon
      expect(dom.find('.react-tree-iconEle').length).to.be(0);
      // show line
      expect(dom.find('.react-tree-line').length).to.be(1);
  ...
  it('should set default expandedKeys, selectedKeys and checkedKeys', () => {
    ...
    expect(li.find('.react-tree-switcher')[0].className).to.contain('open');
    expect(li.find('.react-tree-checkbox-checked').length).to.be(3);
    expect(li.find('.react-tree-node-selected').length).to.be(1);
  });
  ...
  it('should expand specific treeNode', (done) => {
    function cb() {
      setTimeout(() => {
        expect(
          $(instance.refs['treeNode-0-0'].refs.li).find('.react-tree-switcher')[0].className
        ).to.contain('open');
        done();
      });
    }
    ...
    Simulate.click($(instance.refs['treeNode-0-0'].refs.li).find('.react-tree-switcher')[0]);
  });
