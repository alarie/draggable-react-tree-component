  ...
  var loop = function loop(children, level, _parent) {
    ...
    _react2.default.Children.forEach(children, function (item, index) {
      ...
      if (item.props.items && item.type && item.type.isTreeNode) {
        loop(item.props.items, pos, { node: item, pos: pos });
      }

      breakOut = callback(item, index, pos, item.key || pos, getSiblingPosition(index, len, {}), _parent);
    });
  };
...
function filterParentPosition(arr) {
  ...
  var levelArr = (0, _keys2.default)(levelObj).sort();
