...
function loopAllChildren(childs, callback, parent) {
  const loop = function loop(children, level, _parent) {
...
function filterParentPosition(arr) {
  ...
  const levelArr = (0, _keys2.default)(levelObj).sort()
...
function handleCheckState(obj, checkedPositionArr, checkIt) {
  ...
  let objKeys = (0, _keys2.default)(obj)
  // let s = Date.now();
  ...
  const _loop3 = function _loop3(_pIndex) {
    // 循环设置父节点的 选中 或 半选状态
    const loop = function loop(__pos) {
...
function getCheck(treeNodesStates) {
  ...
  (0, _keys2.default)(treeNodesStates).forEach(function (item) {
